import { getLanguageMeta } from '../../languages/index.js';
...
const CompilerHeader = ({ activeLanguage }) => {
  ...
  const subtitle =
    meta.kind === 'sql'
      ? 'SQL Database Editor'
      : meta.kind === 'web'
        ? 'Web (HTML + CSS + JavaScript) Editor'
  ...
  return (
    <header className="compiler-header flex items-center gap-3 px-6 py-5 border-b border-[#e5e7eb] dark:border-dark-border bg-white dark:bg-[#1e1e1e] flex-shrink-0">
      <div className="flex flex-col leading-tight">
        ...
        <span className="text-xs text-[#6b7280] dark:text-gray-400 font-medium">{subtitle}</span>
      </div>
    </header>
  );
};
